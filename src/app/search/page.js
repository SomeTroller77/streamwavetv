...
export default async function Search({searchParams}) {
  ...
  const channelName = (await searchParams).name;
  ...
  return (
    <div>
      <div className="flex flex-col gap-10 px-8 py-6">
        {resp.map((i) => {
          return <Section name={i.name} key={i.id} channels={rawChannels.filter((j) => j.country == country && j.categories?.includes(i.id) && streams.indexOf(j.id) !== -1 && j.name.toLowerCase().includes(channelName.toLowerCase()))} logos={logos}/>
        })}
      </div>
    </div>
  );
